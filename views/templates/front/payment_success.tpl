    ...
    <div class="blockbee_payment-panel {$color_scheme}">
        <div class="blockbee_payment_details">
            <div class="blockbee_payments_wrapper">
                <div class="blockbee_qrcode_wrapper" style="display:{if $qrcode_default}block{else}none{/if}; width: {$qrcode_size}px;">
                    {if $crypto_allowed_value == true}
                        <div class="inner-wrapper">
                            <figure>
                                {if $qr_code_setting != 'hide_without_amount'}
                                    <img class="blockbee_qrcode no_value" {if $qr_code_setting === 'amount'}style="display:none;"{/if} src="data:image/png;base64,{$qr_code_img}"
                                         alt="{l s='QR Code without value' mod='blockbee'}"/>
                                {/if}
                                {if $qr_code_setting != 'hide_amount'}
                                    <img class="blockbee_qrcode value" {if $qr_code_setting === 'without_amount'}style="display:none;"{/if} src="data:image/png;base64,{$qr_code_img_value}"
                                         alt="{l s='QR Code with value' mod='blockbee'}"/>
                                {/if}
                            </figure>
                            {if $qr_code_setting != 'hide_amount' && $qr_code_setting != 'hide_without_amount'}
                                <div class="blockbee_qrcode_buttons">
                                    {if $qr_code_setting != 'hide_without_amount'}
                                        <button class="blockbee_qrcode_btn no_value {if $qr_code_setting === 'without_amount'}active{/if}"
                                                aria-label="{l s='Show QR Code without value' mod='blockbee'}">
                                            {l s='ADDRESS' mod='blockbee'}
                                        </button>
                                    {/if}
                                    {if $qr_code_setting != 'hide_amount'}
                                        <button class="blockbee_qrcode_btn value {if $qr_code_setting === 'amount'}active{/if}" aria-label="{l s='Show QR Code with value' mod='blockbee'}">
                                            {l s='WITH AMOUNT' mod='blockbee'}
                                        </button>
                                    {/if}
                                </div>
                            {/if}
                    ...
                    {else}
                        <div class="inner-wrapper">
                            <figure>
                                <img class="blockbee_qrcode no_value" src="data:image/png;base64,{$qr_code_img}"
                                     alt="{l s='QR Code without value' mod='blockbee'}"/>
                            </figure>
                            <div class="blockbee_qrcode_buttons">
                                <button class="blockbee_qrcode_btn no_value active" aria-label="{l s='Show QR Code without value' mod='blockbee'}">
                                    {l s='ADDRESS' mod='blockbee'}
                                </button>
                            </div>
                        </div>
                ...
                <div class="blockbee_details_box">
                    <div class="blockbee_details_text">
                        {l s='PLEASE SEND' mod='blockbee'}
                        <button class="blockbee_copy blockbee_details_copy" data-tocopy="{$crypto_value}">
                            <span><b class="blockbee_value">{$crypto_value}</b></span>
                            <span><b>{$crypto_coin}</b></span>
                            <span class="blockbee_tooltip blockbee_copy_icon_tooltip tip">{l s='COPY' mod='blockbee'}</span>
                            <span class="blockbee_tooltip blockbee_copy_icon_tooltip success" style="display: none">{l s='COPIED!' mod='blockbee'}</span>
                        </button>
                        <strong>({$currency_symbol} <span class='blockbee_fiat_total'>{$total}</span>)</strong>
                    </div>
                    <div class="blockbee_payment_notification blockbee_notification_payment_received" style="display: none;">
                        {l s='So far you sent' mod='blockbee' }
                        <strong><span class="blockbee_notification_amount"></span></strong>
                        {l s='Please send a new payment to complete the order, as requested above' mod='blockbee'}
                    </div>
                    <div class="blockbee_payment_notification blockbee_notification_remaining" style="display: none">
                        <strong>{l s='Notice' mod='blockbee'}: </strong>{l s='For technical reasons, the minimum amount for each transaction is %1s, so we adjusted the value by adding the remaining to it.' mod='blockbee' sprintf=[$min_tx, '<span class="blockbee_notification_remaining"></span>']}
                    </div>
                    {if $refresh_value_interval != 0}
                        <div class="blockbee_time_refresh">
                            {l s='The %1s conversion rate will be adjusted in' mod='blockbee' sprintf=[$crypto_coin]}
                            <span class="blockbee_time_seconds_count" data-soon="{l s='a moment' mod='blockbee'}"
                                  data-seconds="{$conversion_timer}">{$date_conversion_timer}</span>
                        </div>
                    {/if}
                    <div class="blockbee_details_input">
                        <span>{$address_in}</span>
                        <button class="blockbee_copy blockbee_copy_icon" data-tocopy="{$address_in}">
                            <span class="blockbee_tooltip blockbee_copy_icon_tooltip tip">{l s='COPY' mod='blockbee'}</span>
                            <span class="blockbee_tooltip blockbee_copy_icon_tooltip success" style="display: none">{l s='COPIED!' mod='blockbee'}</span>
                        </button>
                ...
                {if $order_cancelation_timeout != 0}
                    <span class="blockbee_notification_cancel"
                          data-text="{l s='Order will be cancelled in less than a minute.' mod='blockbee'}">{l s='This order will be valid for' mod='blockbee'} <strong><span class="blockbee_cancel_timer" data-timestamp="{$cancel_timer}">{date('H:i', $cancel_timer)}</span></strong></span>
                {/if}
                <div class="blockbee_buttons_container">
                    <a class="blockbee_show_qr" href="#" aria-label="{l s='Show the QR code' mod='blockbee'}">
                        <span class="blockbee_show_qr_open {if !$qrcode_default}active{/if}">
                            {l s='Open QR CODE' mod='blockbee'}
                        </span>
                        <span class="blockbee_show_qr_close {if $qrcode_default}active{/if}">
                            {l s='Close QR CODE' mod='blockbee'}
                        </span>
                    </a>
                </div>
                {if $show_branding}
            ...
            <div class="blockbee_payment_processing" style="display: none;">
                ...
                <h2>{l s='Your payment is being processed!' mod='blockbee'}</h2>
                <h5>{l s='Processing can take some time depending on the blockchain.' mod='blockbee'}</h5>
            </div>
            <div class="blockbee_payment_confirmed" style="display: none;">
                ...
                <h2>{l s='Your payment has been confirmed!' mod='blockbee'}</h2>
            </div>
            <div class="blockbee_payment_cancelled" style="display: none;">
                ...
                <h2>{l s='Order has been cancelled due to lack of payment. Please don\'t send any payment to the address.' mod='blockbee'}</h2>
            </div>
            <div class="blockbee_history" style="display: none;">
                <table class="blockbee_history_fill">
                    <tr class="blockbee_history_header">
                        <th><strong>{l s='Time' mod='blockbee'}</strong></th>
                        <th><strong>{l s='Value Paid' mod='blockbee'}</strong></th>
                        <th><strong>{l s='FIAT Value' mod='blockbee'}</strong></th>
                    </tr>
                </table>
            </div>
            <div class="blockbee_progress">
                <div class="blockbee_progress_icon waiting_payment done">
                    ...
                    <p>{l s='Waiting for payment' mod='blockbee'}</p>
                </div>
                <div class="blockbee_progress_icon waiting_network">
                    ...
                    <p>{l s='Waiting for network confirmation' mod='blockbee'}</p>
                </div>
                <div class="blockbee_progress_icon payment_done">
                    ...
                    <p>{l s='Payment confirmed' mod='blockbee'}</p>
                </div>
            </div>
        </div>
    </div>
